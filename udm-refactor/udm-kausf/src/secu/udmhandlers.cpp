...
void udmhandlers::internalhandler(const std::string& req, std::string& response_send) {
	...
	std::string snn = kvpairs[2];
        ...
        decode_base64(kvpairs[4], akbuf, buflen);

	Authentication_5gaka::derive_kausf(
		ckbuf, ikbuf, snn, sqnbuf, akbuf,
		kausfbuf);  // derive Kausf

        std::string kausf_send;

        encode_base64(kausf_send, kausfbuf, 32);
	std::cout << &kausfbuf << std::endl;	

        response_send = kausf_send;
}
