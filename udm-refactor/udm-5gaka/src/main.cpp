...
void handleaka(const Rest::Request& req, Http::ResponseWriter resp){
	...
	handler1.internalaka(req.body(), response_send);
        resp.send(Pistache::Http::Code::Ok, response_send);

}

void handlef1(const Rest::Request& req, Http::ResponseWriter resp){


        spdlog::debug("Receiving Authentication credentials: {}", req.body());
        std::string response_send;
        udmhandlers handler2;

        //handler2.internalf1(req.body(), response_send);
...
int main(int argc, char* argv[])
{
	...
	Routes::Post(router, "/udmaka", Routes::bind(handleaka));
	Routes::Post(router, "/f1", Routes::bind(handlef1));

	spdlog::info("Starting HTTP Server for UDM-5GAKA Functions");
